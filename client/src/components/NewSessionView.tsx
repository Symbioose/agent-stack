...
export default function NewSessionView({ clis, cli, onCliChange, onSubmit, pending, error }: Props) {
  return (
    <div className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden px-5 pb-[8vh]">
      ...
      <div className="relative w-full max-w-[680px] text-center">
        ...
        <p className="mb-7 mt-2 text-[14px] text-dim max-sm:mx-auto max-sm:max-w-[320px]">
          Démarre un agent sur ta machine. La session continuera même hors ligne.
        </p>
        <Composer clis={clis} cli={cli} onCliChange={onCliChange} onSubmit={onSubmit} pending={pending} />
        {error && (
